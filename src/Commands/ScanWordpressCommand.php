 ...
 */
class ScanWordpressCommand extends Command
{
	/**
	 * @var string
	 */
	protected $signature = 'scan:wordpress';

	/**
	 * @var string
	 */
	protected $name = 'scan:wordpress';
	...
	public function handle()
	{
		$themes = $this->pipeline->send([])
			->through($this->config->get('wordpress.providers'))
			->then(function($resolved_wordpress) {
				return $resolved_wordpress;
			});

		$vulnerable_wordpress = Collection::make($themes)
		...
		if ($vulnerable_wordpress->count() > 0) {
			$this->error('Vulnerable Wordpress Found!');
			$this->table([
				'name', 'title', 'message'
