...
class VulnDB
{
	...
	{
		$requests = Collection::make($plugins)->map(function (Wordpress $plugin) {
			return function () use ($plugin) {
				return $this->http->getAsync("https://wpvulndb.com/api/v2/wordpresses/{$plugin->getSerializedVersion()}/", [
					'exceptions' => false
				]);
			};
		})->getIterator();
		...
		(new Pool($this->http, $requests, [
			...
			'fulfilled' => function ($response, $index) use (&$plugins) {
				if ($response->getStatusCode() === 200) {
					...
					if ($this->vulnDB->isVulnerable($response_object->{$plugins[$index]->getVersion()}->vulnerabilities, $plugins[$index]->getVersion(), $vulnerabilities)) {
